...
class NAPEnv(gym.Env):
    ...
    def general_setting(self,D):
        ...
        if self.do_local_af_opt:
            ...
            if self.kwargs["f_type"] in ["condstringtie", "condscallop", "stringtie", "scallop"]:
                self.cardinality_xi_t = self.kwargs["cardinality_domain"]
                self.xi_t = None
                self.func_X_mean = np.array(self.kwargs["f_opts"]["X_mean"])
                self.func_X_std = np.array(self.kwargs["f_opts"]["X_std"])
                self.func_X_std = np.where(self.func_X_std == 0., 1., self.func_X_std)
            else:
                self.cardinality_xi_t = self.kwargs["cardinality_domain"]
    ...
    def draw_new_function(self):
        ...
        if self.f_type == "GP":
            ...
            #pdb.set_trace()
            #print("generating new HPO function...")
            ...
            self.xi_t = get_stringtie_domain(data=self.pkl_data)
            ...
            max_pos, max, _, min = stringtie_max_min(data=self.pkl_data)
            self.x_max = max_pos
            self.y_max = max
            self.y_min = min
            print(self.x_max, self.y_max, self.y_min)

            self.f = lambda x, idx=None: stringtie(
                x * self.func_X_std[None] + self.func_X_mean[None],
                data=self.pkl_data,
                index=idx
            )
        elif self.f_type == "condstringtie":
            ...
            self.xi_t = get_scallop_domain(data=self.pkl_data)
            ...
            max_pos, max, _, min = scallop_max_min(data=self.pkl_data)
            self.x_max = max_pos
            self.y_max = max
            self.y_min = min
            print(self.x_max, self.y_max, self.y_min)

            self.f = lambda x, idx=None: scallop(
                x * self.func_X_std[None] + self.func_X_mean[None],
                data=self.pkl_data,
                index=idx
            )

        elif self.f_type == "condscallop":
    ...
    def get_state(self, X):
        ...
        if self.kwargs["f_type"] in ["HPO", "condHPO", "condstringtie", "condscallop", "stringtie", "scallop", "Antigen", "condAntigen"]:
            # breakpoint()
            if self.kwargs.get("use_index_speedup", False):
