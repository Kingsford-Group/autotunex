...
nom_dims = []
points = 200
train_datasets = ["./stringtie_"+str(i)+".pkl" for i in range(0,1235)]
train_gp_models = ["./stringtie_"+str(i)+"_gp_model.pt" for i in range(0,1235)]
...
logpath = os.path.join("./HEBO/NAP/", "log/TRAIN", "stringtie", env_spec["env_id"], datetime.strftime(datetime.now(), "%Y-%m-%d-%H-%M-%S"))

if env_spec["f_opts"].get("shuffle_and_cutoff", False) or env_spec["f_type"] in ["condHPO", "HPO", "condstringtie"]:
