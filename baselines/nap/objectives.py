...
def stringtie(x, data, index=None):
    # breakpoint()
    if index is not None:
        ret = data["accs"][index]
        return np.array(ret).reshape(-1, 1)
    else:
        X = get_stringtie_domain(data)
        try:
            idx = np.asscalar(np.where(np.all(x == X, axis=1))[0][0])
            # idx = np.asscalar(np.where(np.all(np.isclose(x, X), axis=1))[0][0])
        except:
            print('[stringtie] except')
            print(x)
            print(np.all(x == X, axis=1))
            print(np.where(np.all(x == X, axis=1)))
            print((np.where(np.all(x == X, axis=1))[0]))
...
def get_stringtie_domain(data):
    return np.array(data["domain"])

def stringtie_max_min(data):
    X = get_stringtie_domain(data)
    idx_max = np.argmax(data["accs"])
    idx_min = np.argmin(data["accs"])
...
def scallop(x, data, index=None):
    # breakpoint()
    if index is not None:
        ret = data["accs"][index]
        return np.array(ret).reshape(-1, 1)
    else:
        X = get_scallop_domain(data)
        try:
            idx = np.asscalar(np.where(np.all(x == X, axis=1))[0][0])
            # idx = np.asscalar(np.where(np.all(np.isclose(x, X), axis=1))[0][0])
        except:
            print('[scallop] except')
            print(x)
            print(np.all(x == X, axis=1))
            print(np.where(np.all(x == X, axis=1)))
            print((np.where(np.all(x == X, axis=1))[0]))
...
def get_scallop_domain(data):
    return np.array(data["domain"])

def scallop_max_min(data):
    X = get_scallop_domain(data)
    idx_max = np.argmax(data["accs"])
    idx_min = np.argmin(data["accs"])
